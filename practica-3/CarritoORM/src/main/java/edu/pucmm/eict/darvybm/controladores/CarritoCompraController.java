        ...
        app.routes(() -> {
            path("/carrito/carritoCompra", () -> {
                ...
                post("/", ctx -> {
                    ...
                    CarritoCompra carritoCompra = ctx.sessionAttribute("carritoCompra");
                    Ventas ventas = new Ventas(usuario.getNombre());
                    for(CarritoItem carritoItem: carritoCompra.getCarritoItems()) {
                        carritoItem.setVenta(ventas);
                        ventas.getCarritoItems().add(carritoItem);
                    }
                    VentasServices.getInstance().addVentas(ventas);
                    ...
                    //Eviando información a la base de datos de CockRoachDB
                    CockroachServices.getInstance().addVenta(ventas);
                    for (CarritoItem carritoItem : ventas.getCarritoItems()){
                        CockroachServices.getInstance().addCarritoItem(carritoItem);
                    }
